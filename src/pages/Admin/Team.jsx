      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[1,2,3].map(i => <div key={i} className="h-12 bg-surface-200 rounded-lg" />)}
          </div>
        </div>
      </div>
    </div>
...
export default function Team() {
  ...
  const handleToggleAvailability = async (washer) => {
    ...
    try {
      ...
      setSaveError(err?.response?.data?.message || "Failed to update availability");
    } finally {
      setSaving(null);
    }
  };
    ...
    <div>
        ...
        <div>
          ...
          {!loading && washers && (
            <p className="text-sm text-surface-400 mt-0.5">
              {washers.length} washer{washers.length !== 1 ? "s" : ""} · {(washers || []).filter(w => w.isAvailable).length} available
            </p>
          )}
        </div>
      ...
      {(error || saveError) && (
        <div className="mb-5 bg-error/8 border border-error/25 rounded-xl px-4 py-3 text-sm text-error flex justify-between">
          {saveError || error}
          <button onClick={() => setSaveError(null)} className="text-error hover:opacity-70">✕</button>
        </div>
      )}
      ...
      {!loading && washers && washers.length > 0 && (
        <div className="flex gap-3 mb-6 overflow-x-auto pb-1">
          {[
            { label: "All",       count: washers.length,                                           color: "bg-surface-200 text-surface-700" },
            { label: "Available", count: washers.filter(w => w.isAvailable).length,               color: "bg-success/15 text-success" },
            { label: "Busy",      count: washers.filter(w => !w.isAvailable).length,              color: "bg-warning/15 text-warning" },
          ].map(({ label, count, color }) => (
            <div key={label} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${color}`}>
              <span>{label}</span>
        ...
        {loading
          ...
          ? (
              <div className="col-span-2 text-center py-16">
                <p className="text-surface-400 text-sm">No washers found. Add your first washer to get started.</p>
              </div>
            )
          : washers.map(w => {
              ...
              return (
                <Card key={w._id} className="p-5 hover:border-surface-300 transition-all">
                  <div className="flex items-start gap-4">
                    {/* Avatar */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize flex-shrink-0 ml-2 ${STATUS_STYLE[statusKey]}`}>
                          {statusKey}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        {isEditingZone ? (
                          <>
                            <input
                              ...
                              onKeyDown={e => e.key === "Enter" && handleZoneSave(w)}
                              className="text-xs border border-primary-500 rounded-lg px-2 py-1 text-surface-700 focus:outline-none w-32"
                              autoFocus
                            />
                            <button
                            ...
                            <button onClick={() => setEditing(null)} className="text-xs text-surface-400 hover:text-surface-600">Cancel</button>
                          </>
                        ) : (
                          <>
                            <span className="text-xs text-surface-500">Zone: <strong className="text-surface-700">{w.zone || "—"}</strong></span>
                            <button
                              onClick={() => { setEditing(w._id); setZoneEdit(w.zone || ""); }}
                        ...
                        <span className="text-xs text-surface-400 ml-auto">⭐ {w.rating} ({w.totalReviews} reviews)</span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-4">
                        {[
                          ...
                          <div key={label} className="bg-surface-100 rounded-lg p-2 text-center">
                            <div className="text-xs text-surface-400">{label}</div>
                            <div className="text-sm font-semibold text-surface-900 mt-0.5">{value}</div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          {w.email && <span className="text-xs text-surface-400">{w.email}</span>}
                        ...
                        >
                          {isSavingThis ? "Saving..." : w.isAvailable ? "Set unavailable" : "Set available"}
                        </button>
                      </div>
                    </div>
                  </div>
