...
function Bar({ label, value, displayValue, max, color = "bg-primary-500", subLabel }) {
    ...
    <div className="mb-4">
      <div className="flex justify-between text-sm mb-1.5">
        <div>
          ...
          {subLabel && <span className="text-surface-400 text-xs ml-2">{subLabel}</span>}
        </div>
        <span className="text-surface-900 font-semibold">{displayValue ?? `${pct}%`}</span>
      </div>
...
export default function Analytics() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl text-surface-900 mb-6">Reports & Analytics</h1>

      {error && (
        <div className="mb-6 bg-error/8 border border-error/25 rounded-xl px-4 py-3 text-sm text-error">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-display text-base text-surface-900">Revenue by service</h3>
            <span className="text-xs text-surface-400">Completed bookings</span>
          </div>
          {loading ? (
            [1,2,3].map(i => <BarSkeleton key={i} />)
          ) : revenueByService.length === 0 ? (
            <p className="text-surface-400 text-sm text-center py-6">No completed bookings yet</p>
          ) : (
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-display text-base text-surface-900">Washes by service</h3>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-display text-base text-surface-900">Popular zones</h3>
            <span className="text-xs text-surface-400 bg-surface-100 px-2 py-0.5 rounded-full">Sample data</span>
          </div>
          {ZONE_DATA.map(z => (
            <Bar
              key={z.zone}
        ...
        {/* Today summary */}
        <Card className="p-5">
          <h3 className="font-display text-base text-surface-900 mb-5">Today at a glance</h3>
          {loading ? (
            <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-12 bg-surface-100 rounded-xl animate-pulse" />)}</div>
          ) : (
            <div className="space-y-3">
              {[
                { label: "New bookings",    value: stats?.today?.bookings ?? 0,   icon: "📋", color: "bg-info/10 text-info" },
                { label: "Completed washes", value: stats?.today?.completed ?? 0, icon: "✅", color: "bg-success/10 text-success" },
                { label: "Active jobs",     value: stats?.today?.activeJobs ?? 0, icon: "🚗", color: "bg-warning/10 text-warning" },
                { label: "Revenue earned",  value: `${(stats?.today?.revenue || 0).toLocaleString()} RWF`, icon: "💰", color: "bg-primary-500/10 text-primary-600" },
              ].map(({ label, value, icon, color }) => (
                <div key={label} className={`flex items-center gap-3 rounded-xl px-4 py-3 ${color.split(" ")[0]}`}>
                  <span className="text-xl">{icon}</span>
                  <span className={`text-sm flex-1 font-medium ${color.split(" ")[1]}`}>{label}</span>
                  <span className={`text-base font-display font-semibold ${color.split(" ")[1]}`}>{value}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {!loading && stats && (
            <div className="border-t border-surface-200 mt-5 pt-4">
              <p className="text-xs text-surface-400 mb-3 font-medium uppercase tracking-wide">All time</p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ["Total bookings", stats.overall?.totalBookings ?? 0],
                  ["Total revenue",  `${((stats.overall?.totalRevenue || 0) / 1000).toFixed(0)}k RWF`],
                ].map(([label, value]) => (
                  <div key={label} className="bg-surface-100 rounded-xl p-3">
                    <div className="text-xs text-surface-400 mb-1">{label}</div>
                    <div className="font-display text-lg text-surface-900">{value}</div>
                  </div>
                ))}
              </div>
            </div>
