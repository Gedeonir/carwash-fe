...
function PreferenceRow({ label, defaultOn }) {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-white/6 last:border-0">
      ...
      >
        <span
          className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-250 ${on ? "left-5" : "left-1"}`}
        />
      </button>
    </div>
  );
...
export default function ProfilePage({ navigate }) {
  ...
  const [notifications, setNotifications] = useState({
    ...
    promos: user?.notifications?.promos,
  });
  ...
  const handleSave = async (data, section = "profile") => {
    setErrors({
      name: null,
      email: null,
      phone: null,
    ...
    if (section === "profile") {
      if (
        !validate("name", data.name, setErrors) ||
        !validate("email", data.email, setErrors) ||
        !validate("phone", data.phone, setErrors)
      )
        return;
    }
    setLoading(true);
  ...
  const totalSpent = HISTORY.filter((h) => h.status === "completed").reduce(
    (s, h) => s + h.amount,
    0,
  );
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col items-center text-center mb-8 bg-surface-50 py-4">
          <div className="relative mb-4">
            {user?.avatar ? (
              ...
              <div className="w-20 h-20 rounded-full bg-surface-500 flex items-center justify-center font-display text-3xl text-primary-50">
                {user?.initials}
              </div>
            )}
          ...
          <p className="text-surface-500 text-sm mb-3">{user?.email}</p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <Card key={label} className="p-4 text-center">
              <div className="font-display text-lg text-surface-900 mb-1">
                {value}
              </div>
        ...
        {tab === "profile" && (
          <div className="flex flex-col gap-4">
                  ...
                  <div key={key}>
                    ...
                    {editing ? (
                      <Input
                        ...
                        // className="w-full bg-surface-100 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-surface-900 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all"
                      />
                    ) : (
            ...
            <Card className="p-5">
              <h3 className="font-display text-base text-surface-900 mb-2">
                Notifications
              </h3>
              <PreferenceRow
                label="Email notifications"
                defaultOn={notifications?.email}
              />
              <PreferenceRow
                label="SMS reminders"
                defaultOn={notifications?.sms}
              />
              <PreferenceRow
                label="Push notifications"
                defaultOn={notifications?.push}
              />
              <PreferenceRow
                label="Promotional offers"
                defaultOn={notifications?.promos}
              />
            </Card>
            ...
            <Card className="p-5">
              ...
              {["Change password", "Privacy settings", "Referral program"].map(
                (item) => (
                  <button
                    key={item}
              ...
              <button
                className="w-full text-left text-sm text-error py-3 mt-1 hover:opacity-80 transition-opacity"
            ...
            {HISTORY.map((h) => (
              <Card key={h.id} className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Badge
                    variant={h.status === "completed" ? "success" : "error"}
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-display text-base text-primary-600">
                    ...
                    <span className="text-xs font-sans text-surface-400">
                      RWF
                    </span>
                  </span>
                </div>
