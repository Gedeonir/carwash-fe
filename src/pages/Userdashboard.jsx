...
const QUICK_SERVICES = [
  ...
  { id: "standard", label: "Standard", price: "10,000", icon: "✨", time: "60 min" },
  { id: "premium",  label: "Premium",  price: "18,000", icon: "🏆", time: "90 min" },
];

const TIPS = [
  { icon: "🌿", title: "Waterless formula", body: "Our products use up to 95% less water than traditional washes." },
  { icon: <Timer color="#0A0A0A"/>, title: "Book 24h ahead", body: "Get your preferred time slot by booking a day in advance." },
  ...
  return (
    <div className="min-h-screen bg-surface-100 pb-28">
        ...
        <div className="mx-auto flex items-center justify-between px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {notifCount > 0 && (
                <span className="absolute top-1.5 right-2 w-2 h-2 bg-primary-500 rounded-full text-[3px] font-bold text-surface-900 flex items-center justify-center">
      ...
      <div className="mx-auto px-5 py-6 space-y-6">
        ...
        <div className="relative bg-surface-50 border border-surface-500/30 rounded-3xl p-5 overflow-hidden">
          ...
          <div className="relative z-10 mb-4">
            <h3 className="font-display text-xl text-surface-900 mb-0.5">{UPCOMING.service}</h3>
            <p className="text-surface-500 text-sm">{UPCOMING.date} · {UPCOMING.time}</p>
          </div>
          ...
          <div className="relative z-10 flex items-center justify-between bg-surface-700/50 rounded-2xl p-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-surface-500">⭐ 4.9 · ETA {UPCOMING.eta}</div>
              </div>
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate("services")} className="text-xs text-primary-600 hover:text-primary-300 transition-colors font-medium">
              See all →
            </button>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {QUICK_SERVICES.map((s) => (
              ...
              >
                ...
                <div className="text-xs font-display text-primary-600">{s.price} <span className="text-surface-500 font-sans">RWF</span></div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate("/profile")} className="text-xs text-primary-600 hover:text-primary-300 transition-colors font-medium">
              View all →
            </button>
          </div>
            ...
            {RECENT.map((r, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-surface-900 truncate">{r.service}</div>
                  <div className="text-xs text-surface-500">{r.date} · {"★".repeat(r.rating)}</div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-sm font-display text-primary-600">{r.amount.toLocaleString()}</div>
          ...
          <div className="relative z-10">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <h3 className="font-display text-base text-white">3 more washes to Gold 🥇</h3>
              </div>
            ...
            <p className="text-xs text-surface-400">7 / 10 washes · Gold unlocks 15% off every booking</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {TIPS.map((tip) => (
              <div key={tip.title} className="flex items-start gap-4 bg-surface-50 border border-white/6 rounded-2xl px-4 py-4">
      ...
      <div className="fixed bottom-4 right-0 z-30 bg-surface-900/95 backdrop-blur-md border-t border-white/6 px-5 pt-3 pb-[calc(72px+env(safe-area-inset-bottom,0px))]">
        <Button className="w-full h-12 shadow-md animate-bounce" onClick={() => navigate("booking")}>
          + Book a New Wash
        </Button>
      </div>
    </div>
